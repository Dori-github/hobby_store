...
<link href="${pageContext.request.contextPath}/css/items.css" rel="stylesheet">
...
<div id="items_main">
	...
	<div id="sidebar" data-param="${param.cate}">
		<ul>
			...
			<c:forEach begin="0" end="2" var="parent">
				<li>
					<a class="cate <c:if test="${param.cate==items_cate[parent].cate_name}">active active-color0</c:if>">${items_cate[parent].cate_name}</a>
					<ul class="d_cate">
	...
	<div id="content">
		...
		<div id="items_search">
			<form action="/items/itemsList.do?check=${param.check}&cate=${param.cate}&packaging=${packaging}" method="post" class="navbar-expand search-form d-flex" id="search_form">
				...
				<input type="search" name="keyword" class="search-bar" id="search_bar" value="${param.keyword}" placeholder="골라봐" autocomplete="off">
		...
		<div id="select">
			...
			<select class="form-select select" id="packaging" name="packaging">
				<option value="1" <c:if test="${param.packaging == 1}">selected</c:if>>포장 주문 여부</option>
				<option value="2" <c:if test="${param.packaging == 2}">selected</c:if>>포장 주문 가능</option>
				<option value="3" <c:if test="${param.packaging == 3}">selected</c:if>>포장 주문 불가능</option>
			</select>
			...
			<select class="form-select select" id="check" name="check">
				...
				<option value="4" <c:if test="${param.check == 4}">selected</c:if>>높은 가격 순</option>
				<option value="5" <c:if test="${param.check == 5}">selected</c:if>>낮은 가격 순</option>
			</select>
		</div>
		...
		<div class="row row-cols-1 row-cols-sm-2 row-cols-md-4 g-4">
			...
			<span  id="red-heart" class = "red-heart" data-num = "${items.items_num}">
			<i class="fa-regular fa-heart"<c:if test ="${fav.fmem_num == user.mem_num && fav.items_num == items.items_num}"> 
						style = "font-weight:bold;"</c:if>></i></span>
			</c:forEach>
			<a href="itemsDetail.do?items_num=${items.items_num}" style="display:block;">
						...
						<div class="card-body">
						  	...
						  	<div class="color-gray">
						  		...
						  		<span class="card-hit"><i class="fa-solid fa-eye"></i> ${items.items_hit} 
